...
`Slice.Wasi` is experimental: its own 0.x APIs may change before a stable release. The upstream WASI build and deployment toolchain is also unstable: `componentize-dotnet`, NativeAOT-LLVM preview packages, WASI Preview 2, `jco`, `preview2-shim`, Wrangler, and host platform behavior can break independently of Slice runtime code. Spin deployment works from the standard `wasi:http/incoming-handler` component produced by this sample; Cloudflare Workers adds a JS transpilation and shim layer, so it has more moving parts.
...
For directories generated by `slice new wasi-cloudflare`, use `npm install` on the first run because the scaffold does not include a lockfile. Review and commit the generated `package-lock.json`, then use `npm ci` for subsequent installs. This checked-in sample already includes `package-lock.json`, so `npm ci` is preferred here.
...
- `SLICE008`, `SLICE009`, `SLICE011`, route binding behavior, and WASI validation results are Slice source-generator or `Slice.Wasi` concerns.
