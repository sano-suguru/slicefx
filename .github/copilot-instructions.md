...
- `src\Slice.Core` contains the runtime framework: `[Feature]`, `[Filter<T>]`, `DataAnnotationsValidationFilter`, and `ISliceValidator<T>`. It must stay dependency-free except for the `Microsoft.AspNetCore.App` framework reference.
...
- Feature assemblies emit module markers so host generators can explicitly aggregate referenced Slice modules without runtime scanning. Aggregation is local-only by default; `SliceReferencedAssemblies` allow-lists referenced assembly simple names, and `SliceAggregateReferences=true` opts into aggregating all directly referenced Slice modules.
