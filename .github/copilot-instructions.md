...
- WASI features should return `WasiResponse`, `SliceResult`, POCOs, `Task<T>`, or `ValueTask<T>`, not `IResult`. `[Filter<T>]` endpoint filters do not run in the WASI path, but `ISliceValidator<T>` validators do. Body-binding WASI routes must provide a source-generated `WasiJsonContext` to avoid `SLICE009`, and WASI DataAnnotations should stay within generated rules (`Required`, `StringLength`, `MinLength`, `MaxLength`, numeric `Range`, `EmailAddress`, `Url`, and `RegularExpression`) to avoid `SLICE011`.
