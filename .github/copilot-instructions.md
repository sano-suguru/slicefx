...
- `src\SliceFx.Core` contains the runtime framework: `[Feature]`, `[Filter<T>]`, `ISliceValidator<T>`, and `SliceValidationResult`. It must stay dependency-free except for the `Microsoft.AspNetCore.App` framework reference.
...
- Generated and runtime endpoint registration order is significant: map method, attach generated DataAnnotations validation when needed, attach generated `ISliceValidator<T>` validation, attach `[Filter<T>]` filters in declaration order, then apply tags, endpoint name, and summary.
...
- `[Filter<T>]` types are plain ASP.NET Core `IEndpointFilter`s. `AddSlice()` registers referenced filters as scoped services. Generated DataAnnotations validation always runs before feature filters.
- Filter declaration order is meaningful. `FilterOrderHintAttribute` currently enforces `After = typeof(...)` preferences at compile time with `SLICE007`.
...
- WASI features should return `WasiResponse`, `SliceResult`, POCOs, `Task<T>`, or `ValueTask<T>`, not `IResult`. `[Filter<T>]` endpoint filters do not run in the WASI path, but `ISliceValidator<T>` validators do. Body-binding WASI routes must provide a source-generated `WasiJsonContext` to avoid `SLICE021`, and WASI DataAnnotations should stay within generated rules (`Required`, `StringLength`, `MinLength`, `MaxLength`, numeric `Range`, `EmailAddress`, `Url`, and `RegularExpression`) to avoid `SLICE022`.
