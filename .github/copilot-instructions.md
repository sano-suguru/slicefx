...
- `src\Slice.SourceGenerator` is the sole registration path. It discovers `[Feature]` classes, validates route/handler shape with `SLICE###` diagnostics, and emits `AddSlice`/`MapSlices` extension methods into the `Slice` namespace. `RegistrationEmitter` is the authoritative source for registration logic.
