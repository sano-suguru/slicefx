...
internal static partial class GenerateTypeScriptClientCommand
{
    ...
    private static string RenderClient(string className, SliceRouteInfo[] routes, FileInfo[] assemblyFiles)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("export interface SliceProblemDetails {");
        ...
        sb.AppendLine("}");
        sb.AppendLine();
        sb.AppendLine("export class SliceApiError extends Error {");
        sb.AppendLine("  readonly status: number;");
        sb.AppendLine("  readonly problem: SliceProblemDetails | undefined;");
        sb.AppendLine("  constructor(message: string, status: number, problem?: SliceProblemDetails) {");
        sb.AppendLine("    super(message);");
        sb.AppendLine("    this.name = 'SliceApiError';");
        sb.AppendLine("    this.status = status;");
        sb.AppendLine("    this.problem = problem;");
        sb.AppendLine("  }");
        sb.AppendLine("}");
        sb.AppendLine();
        ...
        IReadOnlyDictionary<string, TypeSchema> typeSchemas)
    {
        ...
        sb.AppendLine("      let __problem: SliceProblemDetails | undefined;");
        sb.AppendLine("      const __ct = response.headers.get('content-type') ?? '';");
        sb.AppendLine("      if (__ct.includes('json')) {");
        sb.AppendLine("        try { __problem = await response.json() as SliceProblemDetails; } catch { try { await response.body?.cancel(); } catch { /* ignore */ } }");
        sb.AppendLine("      } else {");
        sb.AppendLine("        await response.body?.cancel();");
        sb.AppendLine("      }");
        sb.AppendLine(CultureInfo.InvariantCulture,
            $"      throw new SliceApiError(`{methodName}Async failed: ${{response.status}} ${{response.statusText}}`, response.status, __problem);");
        sb.AppendLine("    }");
