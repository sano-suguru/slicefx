...
- **Host-neutral filters (`[SliceFilter<T>]` / `ISliceFilter`)** run on both ASP.NET and WASI paths. `ISliceFilter.InvokeAsync` receives a `SliceFilterContext` with read-only request metadata (`Method`, `Path`, `Headers`, `RouteValues`, `Services`, `CancellationToken`) and a writable `ResponseHeaders` dictionary (`IDictionary<string, string>`, case-insensitive). Entries written to `ResponseHeaders` are merged into the host response before it is sent (ASP.NET: via `Response.OnStarting`; WASI: by rebuilding `WasiResponse.Headers`). Handler-set headers take priority; `ResponseHeaders` only adds keys not already set by the handler. Single-valued headers only — for multi-valued headers (e.g., `Set-Cookie`) use `IEndpointFilter` on the ASP.NET path. `[Filter<T>]` (ASP.NET `IEndpointFilter`) is not executed on the WASI path; `[SliceFilter<T>]` (`ISliceFilter`) is. Lambda function-per-feature (FPF) path does not execute neutral filters.
