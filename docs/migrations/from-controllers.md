...
Supported generated rules include `Required`, `StringLength`, `MinLength`, `MaxLength`, numeric `Range`, `EmailAddress`, `Url`, and `RegularExpression`. Reflection-bound validation such as custom `ValidationAttribute`, type-level validation, `IValidatableObject`, and resource-based messages is reported at build time for generated ASP.NET registrations and excluded from portable WASI/Lambda function-per-feature dispatch. Move those rules to `ISliceValidator<TRequest>` when migrating an action.
