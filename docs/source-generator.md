...
| `SLICE021` | Warning | WASI portability | WASI JSON serialization metadata is missing or incomplete for this feature. When a `[SliceJsonContext(Wasi)]` context exists with at least one `[JsonSerializable]` entry, the diagnostic lists the specific missing type(s) by name. When no context exists, the entire feature is excluded. | Provide a `JsonSerializerContext` with `[SliceJsonContext(SliceJsonTarget.Wasi)]` and register all required body/response types. |
...
| `SLICE071` | Error | ASP.NET NativeAOT-safe registration | Feature body or response type is missing from the `[SliceJsonContext(SliceJsonTarget.AspNet)]` context. When the context exists with at least one `[JsonSerializable]` entry, the diagnostic lists each missing type by name (per-type detection). When no context exists, all required types are listed. | Add `[JsonSerializable(typeof(T))]` for each missing type, or run `slicefx json-context --fix` to insert the entries automatically. |
