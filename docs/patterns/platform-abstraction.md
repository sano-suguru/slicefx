...
**WASI `IncomingHandlerExportsImpl.cs`:**
```csharp
using SliceFx.Wasi;
...
**Capability implementation cost:** Each capability satellite has two layers of cost. The first layer — the thin pure-interface package (1–5 methods) plus its DI extension and in-memory test double — is written once and reused. The second, heavier layer — a WIT-bound application implementation and, for server-side exports such as cron, an entry-point bridge that calls `SpinCronDispatcher.DispatchAsync` — is host-specific and lives in the application. The first layer is demonstrated end-to-end in `samples/SliceFx.WasiSample` (the `Notes` features use `IKeyValueStore`, `POST /fetch` uses `IWasiHttpClient`, both wired to the in-memory doubles) and covered by in-process unit tests (`SliceFx.Wasi.KeyValue.Tests`, `SliceFx.Wasi.HttpClient.Tests`, `SliceFx.Wasi.Spin.Tests`); `SliceFx.Wasi.Spin` additionally carries `README.md` sample code. The second, WIT-bound layer is documented via XML doc-comments on each interface. The reference WIT-bound application wiring for all four capabilities lives in the [`slicefx-inbox`](https://github.com/sano-suguru/slicefx-inbox) dogfooding app, which runs them in production on Fermyon Cloud / Spin (KV multi-tenant store, outbound HTTP for `og:title` fetch, cron-driven feed refresh, Spin variables). The satellite interfaces themselves ship no host-specific implementation in this repository — treat them as interface specifications, with `slicefx-inbox` as the worked example of the second layer.
